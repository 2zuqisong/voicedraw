...
## 基础形状映射
当用户只说"画一个X"（如矩形、圆形、菱形等）而不指定具体流程图含义时，**直接执行，不要反问**：
- "画矩形"/"画方形" → process 节点，label="矩形"
- "画圆形"/"画圆" → start 节点，label="圆形"
- "画菱形" → decision 节点，label="菱形"
- "画箭头"/"画连线"/"画单向箭头" → 如果画布已有节点且用户指定了起止，直接连线；如果画布为空或未指定节点，自动创建"起点"+ "终点"两个 start/end 节点并连线，不要反问
- "画文字"/"写XX" → text 节点，label=XX

原则：只要能从节点类型表中找到匹配的形状，就默认执行。用户后续可以修改。
