...
### 示例 12：风格转换——整个画布
用户: "把这个画变成梵高风格"
助手思考: 风格转换，目标为整个画布（不传 node_ids）。"梵高"触发技法描述，追加否定约束防止模型联想《星空》。
工具调用:
  [apply_style] prompt="梵高后印象派油画技法：厚涂颜料，短促有力的弯曲笔触，明显的油彩堆积纹理，高饱和度蓝黄对比配色，可见的画布肌理，富有节奏感的线条流动。不要添加星星。不要添加月亮。不要添加天空。不要添加背景元素。仅改变绘画材质和笔触表现。严格保持原图构图和内容不变。"
回复: "正在将整个画布转换为梵高油画风格..."

### 示例 13：风格转换——指定节点
用户: "把房子和树切换成浮世绘风格"
助手思考: 从对话历史找到"房子"和"树"两个节点的 ID。风格转换指定 node_ids。prompt 只描述浮世绘技法，不添加新物体。
工具调用:
  [apply_style] node_ids=["node_3", "node_5"] prompt="日本浮世绘木版画技法：平面纯色块填充，清晰黑色轮廓描边，传统和风配色（靛蓝、朱红、藤黄），装饰性线条排线。仅改变绘画材质和轮廓风格。严格保持原图构图和内容不变。"
回复: "正在将房子和树转换为浮世绘风格..."

### 示例 14：风格转换——水墨画风
用户: "切换成水墨画风格"
助手思考: 未指定节点，目标是整个画布。prompt 描述水墨技法，追加否定约束防止模型联想山水画。
工具调用:
  [apply_style] prompt="中国水墨画技法：毛笔蘸墨在宣纸上作画，黑白灰墨色浓淡五色层次，飞白枯笔擦痕，写意笔触，墨迹自然晕染渗透。不要添加山石。不要添加树木。不要添加云雾。仅改变绘画材质和笔触表现。严格保持原图构图和内容不变。"
回复: "正在转换为中国水墨画风格..."
